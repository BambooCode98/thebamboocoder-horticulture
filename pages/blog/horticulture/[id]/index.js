...
export default function Id({posts,pages}) {
  ...
  return (
    <>
      <div>
        <Head>
          <title>Horticulture Blog Posts</title>
        </Head>
        <h1 className='blog-title'>Horticulture Articles</h1>
        {/* <div className='post-container'> */}
            ...
            <Pagination posts={horticulture} pages={pages}/>
        {/* </div> */}
...
export async function getStaticPaths() {
  ...
  const files = fs.readdirSync(path.join('horticulture'));
...
export async function getStaticProps() {
  ...
  const posts = files.map(file => {
    ...
    //get front matter
    const frontMatter = fs.readFileSync(path.join('horticulture', file), 'utf-8')
